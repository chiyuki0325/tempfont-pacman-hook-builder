...
function generate_hook(program){
    return(`#!/bin/bash
bwrap --dev-bind / / --tmpfs ~/.fonts/ms-tmp /bin/bash -c "for file in $(find ${hidden_fonts_dir} -name '.*' | grep tt | sed -s 's#${hidden_fonts_dir}/.##');do cp ${hidden_fonts_dir}/ms/.\\$file ${temp_fonts_dir}/\\$file;done;ls ${temp_fonts_dir};/usr/bin/${program}.bak \"$@\""`);
};
